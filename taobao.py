...
def main():
    driver = webdriver.PhantomJS(executable_path='/usr/local/Cellar/phantomjs/2.1.1/bin/phantomjs')#浏览器的地址
    driver.get("https://mm.taobao.com/search_tstar_model.htm?")#目标网页地址
    bsObj=BeautifulSoup(driver.page_source,"lxml")#解析html语言
    fp = open('/users/chengjunjie/Desktop/mm.txt','r+')#用来将主页上的个人信息存储
    ...
    while(i<5):
        ...
        path = '/users/chengjunjie/Desktop/Photo/'+contents[i][0][0]
        mkdir(path)#建立相应文件夹
    ...
    for imageUrl in imagesUrl:#将封面图片存入对应文件夹中
        ...
        html = urlopen(url)
        data = html.read()
        fileName = '/users/chengjunjie/Desktop/Photo/封面/'+str(number)+'.jpg'
...
def getperMMpageImg(MMURL,MMpath):
    owndriver = webdriver.PhantomJS(executable_path='/usr/local/Cellar/phantomjs/2.1.1/bin/phantomjs')
    owndriver.get(MMURL)
    print("[*]Opening.....MM....................")
